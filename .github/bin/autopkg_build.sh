...
# RHEL
    ...
    if [ "$OMJ_BUILDER" == rpmbuild ]; then
        ...
        echo rpmbuild -ba --define "_topdir $TOPDIR" --with check --with full --without tiny --build-in-place "$SPEC"
        rpmbuild -ba --define "_topdir $TOPDIR" --with check --without tiny --build-in-place "$SPEC"
