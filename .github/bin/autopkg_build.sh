...
# RHEL
    ...
    if [ "$OMJ_BUILDER" == rpmbuild ]; then
        ...
        echo rpmbuild -ba --define "_topdir $TOPDIR" --without check --with full --without tiny --build-in-place "$SPEC"
        rpmbuild -ba --define "_topdir $TOPDIR" --without check --without tiny --build-in-place "$SPEC"
